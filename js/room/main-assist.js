...
// URL params — avatar + vrm are opaque uids (8-char hex). No numeric/server accepted.
const urlParams = new URLSearchParams(window.location.search);
const paramAvatarUid = urlParams.get('avatar') || '';
const vrmUid = urlParams.get('vrm');

// avatar uid must be exactly 8 hex chars
if (!/^[0-9a-f]{8}$/.test(paramAvatarUid)) {
  console.warn('[MainAssist] Invalid avatar uid, redirecting:', paramAvatarUid);
...
// Serialize: resolve avatar uid → numeric id, then load VRM (wires chat via vrm-loader)
(async () => {
  try {
    ...
    console.error('[MainAssist] avatar uid resolve failed:', e);
    location.replace('index.html');
    return;
  }
