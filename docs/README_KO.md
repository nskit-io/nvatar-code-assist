...
메시지가 Claude Code로 비동기 전달됩니다. **타임아웃 없음** — 아바타가 접수를 바로 알려주고, 결과가 오면 자동 전달합니다:

| 동작 | 처리 방식 |
|------|-----------|
| 사용자 메시지 | Claude Code로 전달 → 아바타가 "클로드에게 의뢰했어요!" 안내 |
| 진행 상황 업데이트 | 아바타가 실시간으로 음성 전달 |
| 최종 결과 | 아바타 음성 전달 + Code Panel 표시 |
| 다이렉트 푸시 | Claude Code가 아바타 요청 없이도 결과 전송 가능 |
| 아바타 의견 요청 | Gemma가 결과 컨텍스트와 함께 응답 |

> **Fire-and-forget**: 긴 작업에서도 타임아웃 실패가 없습니다. 서버는 POST 전달 확인 후 즉시 반환하고, 결과는 비동기 push로 도착합니다.
...
Code Assist는 여러 계층의 재시도 및 오류 처리를 포함합니다:

| 계층 | 동작 |
|------|------|
| **채널 → Claude Code** | MCP 알림 재시도 (3회, 2초 간격) |
| **서버 → 채널** | HTTP POST 재시도 (2회, 2초 간격) |
| **전달 실패** | 단순 오류 말풍선 표시 — 아바타가 실패에 대해 창작 응답을 생성하지 않음 |
| **TTS 초기화** | `client_ready` 핸드셰이크로 TTS 초기화 후 아바타가 발화 |

재시도 후에도 Claude Code에 메시지가 전달되지 않으면, 로컬 AI(Gemma)로 라우팅하는 대신 단순 오류 메시지를 표시합니다. 이렇게 하면 아바타가 코드 명령에 대해 엉뚱한 답변을 "지어내는" 혼란을 방지할 수 있습니다.
...
| 아바타가 발화 시 중앙으로 이동하지 않음 | 수정 완료 — 모든 말풍선 이벤트에서 `returnToCenter()`가 호출됩니다 |
| TTS 준비 전에 아바타가 말함 | 수정 완료 — `client_ready` 핸드셰이크로 씬 로딩 후 인사를 지연합니다 |
| 메시지 전달 실패가 조용히 발생함 | 서버가 2초 간격으로 2회 재시도 후, 최종 실패 시 오류를 표시합니다 |
