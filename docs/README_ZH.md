...
NVatar Code Assist使用**Claude Code Channel** — 基于MCP的插件系统，让外部应用与运行中的Claude Code会话进行通信。
...
`channel/`目录包含一个即用型Bun MCP服务器。无需额外配置，安装后直接运行：

```bash
cd channel && bun install          # 仅需一次
NVATAR_CHANNEL_UUID=<uuid> claude --dangerously-load-development-channels server:nvatar
```

Claude Code会自动启动频道服务器。频道通过HTTP向NVatar服务器注册，并双向中继消息。

> 有关Claude Code Channel和MCP的详细信息，请参阅[Claude Code文档](https://docs.anthropic.com/en/docs/claude-code)。
...
> **说明:** NVatar服务器托管在`https://nvatar.nskit.io`。不支持本地自建部署。如需在自有基础设施上部署NVatar，请[联系我们](mailto:nskit@nskit.io)。
...
消息异步转发至Claude Code — **无超时限制**。虚拟形象立即确认接收，结果到达后自动推送:

| 操作 | 行为 |
|------|------|
| 用户消息 | 转发至Claude Code → 虚拟形象提示"已经把任务交给Claude了！" |
| 进度更新 | 虚拟形象实时语音播报 |
| 最终结果 | 虚拟形象语音播报 + Code Panel显示 |
| 直接推送 | Claude Code可以在没有虚拟形象请求的情况下推送结果 |
| 询问虚拟形象意见 | Gemma结合结果上下文进行回复 |

> **Fire-and-forget**: 长时间任务不会超时失败。服务器确认POST投递后立即返回，结果通过异步push到达。

### 隐私与数据

> **重要:** Code Assist模式下的对话不会保存到虚拟形象的记忆中。
...
| **情感追踪** | 启用（喜悦、悲伤等） | 停用 |
| **性格发展** | 启用（随时间变化） | 停用 |
| **代码结果** | 不适用 | 保存到Code Panel + SQLite（独立于虚拟形象记忆） |

日常对话会促进虚拟形象的性格和情感发展。Code Assist对话完全隔离，不会影响虚拟形象的角色或记忆。切换回普通模式时，虚拟形象会从之前的日常对话继续。
...
Code Assist内置了多层重试与错误处理机制:

| 层级 | 行为 |
|------|------|
| **通道 → Claude Code** | MCP通知重试（3次，间隔2秒） |
| **服务器 → 通道** | HTTP POST重试（2次，间隔2秒） |
| **投递失败** | 显示简单错误气泡 — 虚拟形象不会对失败生成创作性回复 |
| **TTS初始化** | `client_ready`握手确保TTS初始化完成后虚拟形象才开始说话 |

当重试后消息仍无法到达Claude Code时，系统会显示简单的错误提示，而不是将请求路由到本地AI（Gemma）。这样可以防止虚拟形象对代码指令"编造"答案，避免产生令人困惑的回复。

## 服务限制

| 服务 | 端点 | 说明 |
|------|------|------|
| **NVatar服务器** | `nvatar.nskit.io` | 公共托管服务器。不支持自建部署。 |
| **TTS（语音）** | ElevenLabs via `nvatar.nskit.io` | 可能因API配额限流。TTS不可用时，虚拟形象通过文字气泡正常工作。 |
| **STT（语音识别）** | `whisper.nskit.io`（本地Whisper） | 自托管，运行稳定。故障时请使用文字输入。 |

- TTS使用共享的ElevenLabs API配额。高峰期语音输出可能暂时中断，但聊天和代码中继通过文字气泡正常运作。
- 如需专用资源的企业部署，请[联系我们](mailto:nskit@nskit.io)。
...
| 虚拟形象说话时不移动到中央 | 已修复 — 每个气泡事件都会调用`returnToCenter()` |
| TTS就绪前虚拟形象就开始说话 | 已修复 — `client_ready`握手会延迟问候直到场景加载完成 |
| 消息投递静默失败 | 服务器现以2秒间隔重试2次；最终失败时显示错误提示 |
